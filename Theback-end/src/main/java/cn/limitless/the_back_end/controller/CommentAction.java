...
public class CommentAction {
	...
	public ObjectModel deleteComment(@ApiParam(value = "评论的id", required = true) @RequestParam(name = "id") String commentOrderId) {
		final ObjectModel objectModel = new ObjectModel();
		final Comment comment = this.commentService.queryCommentByOrderId(commentOrderId);
		if (commentOrderId == null || "".equals(commentOrderId)) {
			objectModel.error();
		} else {
			final boolean b = this.commentService.deleteComment(comment.getCommentId());
			if (!b) {
				objectModel.error();
			}
		}
